...
export default function CustomerView({ menu }) {
  ...
  const trackOrder = async () => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/orders/${trackingId}`);
      if (res.ok) {
  ...
  if (orderConfirmation) {
    return (
      <div data-testid="order-confirmation">
        ...
        {/* Displaying payment status as requested */}
        <p>Payment Status: {orderConfirmation.paymentStatus}</p>
        <p>
          Final Price: {orderConfirmation.totalPrice} NIS (Calculated by Server)
        </p>
        <button
  ...
  return (
    <div>
        ...
        {menu.pizzas.map((p) => (
          <label key={p.id} style={{ display: "block" }}>
            <input
              type="radio"
              name="pizza"
              checked={selectedPizza.id === p.id}
              onChange={() => setSelectedPizza(p)}
            />
            {p.name} ({p.price} NIS)
          </label>
        ))}
      ...
      {/* New section for tracking orders */}
      <div>
        ...
        {trackingError && <p style={{ color: "#c0392b" }}>{trackingError}</p>}
        {trackedOrder && (
          <div
            style={{
              padding: "15px",
          ...
          >
            ...
            <p>
              <strong>Payment Status:</strong> {trackedOrder.paymentStatus}
            </p>
            <p>
              <strong>Total Price:</strong> {trackedOrder.totalPrice} NIS
            </p>
          </div>
        )}
      </div>
